...
-- Load Custom Gojo-Themed Notification System
local TweenService = game:GetService("TweenService")
local GOJO_BLUE = Color3.fromRGB(0, 191, 255)
local GOJO_DARK_BLUE = Color3.fromRGB(0, 100, 255)
local GOJO_LIGHT_BLUE = Color3.fromRGB(135, 206, 250)

getgenv().Notify = function(config)
    local player = game:GetService("Players").LocalPlayer
    local playerGui = player:WaitForChild("PlayerGui")
    local screenGui = playerGui:FindFirstChild("TelelumiNotifications")
    if not screenGui then
        screenGui = Instance.new("ScreenGui")
        screenGui.Name = "TelelumiNotifications"
    ...
    gradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.fromRGB(0, 0, 0)), ColorSequenceKeypoint.new(0.5, Color3.fromRGB(5, 30, 60)), ColorSequenceKeypoint.new(1, Color3.fromRGB(0, 0, 0))}
    ...
    titleBar.BackgroundColor3 = GOJO_DARK_BLUE
    ...
    titleGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, GOJO_DARK_BLUE), ColorSequenceKeypoint.new(0.5, GOJO_BLUE), ColorSequenceKeypoint.new(1, GOJO_DARK_BLUE)}
    ...
    titleLabel.Text = config.Title or "Telelumi Animation Menu"
    titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
    titleLabel.TextSize = 16
    titleLabel.TextXAlignment = Enum.TextXAlignment.Left
    titleLabel.TextStrokeTransparency = 0.5
    titleLabel.TextStrokeColor3 = GOJO_BLUE
    ...
    spawn(function()
        ...
        while notification.Parent do
            if increasing then
                ...
                if transparency >= 0.8 then increasing = false end
            else
                transparency = transparency - 0.05
                if transparency <= 0.3 then increasing = true end
            end
            titleLabel.TextStrokeTransparency = transparency
            task.wait(0.1)
        end
    end)
    ...
    contentLabel.TextColor3 = GOJO_LIGHT_BLUE
    contentLabel.TextSize = 14
    contentLabel.TextWrapped = true
    contentLabel.TextXAlignment = Enum.TextXAlignment.Left
    contentLabel.TextYAlignment = Enum.TextYAlignment.Top
    contentLabel.Parent = notification
    local textBounds = contentLabel.TextBounds.Y
    local finalHeight = math.max(80, textBounds + 50)
    notification.Size = UDim2.new(0, 350, 0, 0)
    notification.Position = UDim2.new(1, -370, 0, 20)
    local slideIn = TweenService:Create(notification, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(0, 350, 0, finalHeight)})
    slideIn:Play()
    spawn(function()
        local glowIn = TweenService:Create(stroke, TweenInfo.new(0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {Transparency = 0.3, Thickness = 3})
        local glowOut = TweenService:Create(stroke, TweenInfo.new(0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {Transparency = 0, Thickness = 2})
        while notification.Parent do
            glowIn:Play()
            task.wait(0.8)
    ...
    closeButton.MouseButton1Click:Connect(function()
        local slideOut = TweenService:Create(notification, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.In), {Size = UDim2.new(0, 350, 0, 0)})
        slideOut:Play()
    ...
    for _, child in pairs(screenGui:GetChildren()) do
        if child:IsA("Frame") and child.Name == "Notification" and child ~= notification then
            yOffset = yOffset + child.Size.Y.Offset + 10
        end
    end
    notification.Position = UDim2.new(1, -370, 0, yOffset)
end
...
Changepage.ZIndex = 3
-- INSTRUCTIONS: Upload the Gojo image file to Roblox as a decal/image
-- Then replace the number below with your asset ID
-- The image file is saved in the same folder as this script
Changepage.Image = "rbxassetid://YOUR_ASSET_ID_HERE"
